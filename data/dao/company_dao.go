...
func (dao *CompanyDAO) Create(company *entities.Company) error {
	_, err := dao.Connection.ExecContext(
		...
		VALUES ($1, $2, $3, $4, $5, $6)`,
		company.Id, company.Name, company.Activate,
		company.CountryId, company.CountrySubdivisionId, company.CompanyGroupId,
	)
	return err
}
...
func (dao *CompanyDAO) Update(company *entities.Company) error {
	_, err := dao.Connection.ExecContext(
		...
		WHERE id = $6`,
		company.Name, company.Activate, company.CountryId,
		company.CountrySubdivisionId, company.CompanyGroupId, company.Id,
	)
	return err
}
